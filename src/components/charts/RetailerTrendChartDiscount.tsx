...
function processRetailerDiscountData(resultSet: any, granularity: string = "day") {
  if (!resultSet) return [];
  ...
  try {
    ...
    console.error("Error processing retailer discount data:", error);
    return [];
  }
}
...
export function RetailerTrendChartDiscount({
  ...
  const data = useMemo(() => {
    return processRetailerDiscountData(resultSet, globalFilters.granularity);
