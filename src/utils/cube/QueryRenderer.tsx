...
export function QueryRenderer(props: QueryRendererProps) {
  ...
  if (isLoading) {
    return showSkeleton ? <ChartAreaSkeleton /> : (
  ...
  if (error) {
    return <div className="text-red-500 p-4 border border-red-300 rounded bg-red-50 dark:bg-red-950/20">
  ...
  if (!resultSet) {
    return <div className="text-muted-foreground p-8 text-center border rounded-lg bg-muted/20">
      No data available
    </div>;
  }
